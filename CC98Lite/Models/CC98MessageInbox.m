...
@implementation CC98MessageInbox
...
- (UIImage *)icon {
    return [UIImage imageNamed:@"inbox"];
}
...
- (void)messagesInPage:(NSInteger)pageNum withBlock:(void (^)(NSArray *messages, NSError *error))block {
    [[CC98Client sharedInstance] GET:[NSString stringWithFormat:@"usersms.asp?action=inbox&page=%ld", (long)pageNum]
                          parameters:nil
        ...
        for (NSString *match in matches) {
            ...
            message.hasRead = [[modified firstMatchRegex:@"(?<=<img src=pic/).*?(?=\\.gif>)"] isEqualToString:@"m_olds"] ? YES : NO;
